...
const VisitorDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const appointmentRes = await axios.get(`${API_URL}/appointments`, {
          headers,
        });

        setAppointments(appointmentRes.data || []);

        const passRes = await axios.get(`${API_URL}/passes`, { headers });

        setPasses(passRes.data || []);

        console.log(JSON.stringify(passRes.data, null, 2));
  ...
  const getPdfUrl = (filename) => {
    ...
    return `${backendBaseUrl}/uploads/passes/${filename}`;
  };
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ fontSize: "32px", margin: 0 }}>Visitor Dashboard</h2>
        ...
        {appointments.length === 0 ? (
          ...
          appointments.map((appointment) => (
            ...
            >
              ...
              <p>👤 Host: {appointment.hostId?.name || "Not Assigned"}</p>
      ...
      {/* PASSES SECTION */}
      <div>
        <h3 style={{ marginBottom: "20px" }}>My Digital Passes</h3>
              ...
              (
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p>Host: {pass.appointmentId?.hostId?.name}</p>

                    <p>Purpose: {pass.appointmentId?.purpose}</p>

                    <p>Expires: {new Date(pass.expiryDate).toLocaleString()}</p>
